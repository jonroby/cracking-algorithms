...
describe('BloomFilter', function () {
  var bloomFilter = void 0;
  var people = ['Bruce Wayne', 'Clark Kent', 'Barry Allen'];

  beforeEach(function () {
    bloomFilter = new _BloomFilter2.default();
  });

  it('should have methods named "insert" and "mayContain"', function () {
    expect(_typeof(bloomFilter.insert)).toBe('function');
    expect(_typeof(bloomFilter.mayContain)).toBe('function');
  });

  it('should create a new filter store with the appropriate methods', function () {
    var store = bloomFilter.createStore(18);
    expect(_typeof(store.getValue)).toBe('function');
    expect(_typeof(store.setValue)).toBe('function');
  });
  ...
  it('should insert strings correctly and return true when checking for inserted values', function () {
    people.forEach(function (person) {
      return bloomFilter.insert(person);
    });
