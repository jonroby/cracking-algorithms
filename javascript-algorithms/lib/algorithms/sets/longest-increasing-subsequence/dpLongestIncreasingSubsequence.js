...
exports.default = dpLongestIncreasingSubsequence;
/**
...
function dpLongestIncreasingSubsequence(sequence) {
  ...
  var lengthsArray = Array(sequence.length).fill(1);
  ...
  while (currentElementIndex < sequence.length) {
    if (sequence[previousElementIndex] < sequence[currentElementIndex]) {
      ...
      var newLength = lengthsArray[previousElementIndex] + 1;
  ...
  var longestIncreasingLength = 0;

  for (var i = 0; i < lengthsArray.length; i += 1) {
