...
exports.default = dpBottomUpJumpGame;
/**
...
function dpBottomUpJumpGame(numbers) {
  ...
  var cellsGoodness = Array(numbers.length).fill(undefined);
  ...
  for (var cellIndex = numbers.length - 2; cellIndex >= 0; cellIndex -= 1) {
    var maxJumpLength = Math.min(numbers[cellIndex], numbers.length - 1 - cellIndex);

    for (var jumpLength = maxJumpLength; jumpLength > 0; jumpLength -= 1) {
      var nextIndex = cellIndex + jumpLength;
