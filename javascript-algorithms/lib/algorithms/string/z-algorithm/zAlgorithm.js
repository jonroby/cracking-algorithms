...
function buildZArray(zString) {
  ...
  var zArray = new Array(zString.length).fill(null).map(function () {
    return 0;
  });
...
function zAlgorithm(text, word) {
  ...
  var zString = '' + word + SEPARATOR + text;

  // Generate Z-array for concatenated string.
  var zArray = buildZArray(zString);
  ...
  for (var charIndex = 1; charIndex < zArray.length; charIndex += 1) {
